...
if [ "${AITER_DSV4_PR2998:-1}" = "1" ]; then
    ...
    AITER_PR2998_SHA=${AITER_PR2998_SHA:-126e40176dd5d09d0c472c182aad24ae69f8b4a4}
...
if [ "${ATOM_DSV4_PR650:-1}" = "1" ]; then
    ...
    ATOM_PR650_SHA=${ATOM_PR650_SHA:-e00021d2a09e1f64c44b57e988ea5d20b1cc4023}
...
if [ "${EVAL_ONLY}" = "true" ]; then
    ...
    export AITER_DSV4_MOE_TRACE_SET="${AITER_DSV4_MOE_TRACE_SET:-0}"
    export AITER_DSV4_MOE_ROW_CHECK="${AITER_DSV4_MOE_ROW_CHECK:-0}"
    export AITER_DSV4_MOE_CANARY="${AITER_DSV4_MOE_CANARY:-0}"
    export AITER_DSV4_MOE_TRACE_RAW_SCALE="${AITER_DSV4_MOE_TRACE_RAW_SCALE:-0}"
    export AITER_DSV4_MOE_ABORT_AFTER_SCALE_TRACE="${AITER_DSV4_MOE_ABORT_AFTER_SCALE_TRACE:-}"
    export AITER_DSV4_MOE_TRACE_REQUIRE_PRODLIKE="${AITER_DSV4_MOE_TRACE_REQUIRE_PRODLIKE:-0}"
    export AITER_DSV4_MOE_PRODLIKE_MIN_M="${AITER_DSV4_MOE_PRODLIKE_MIN_M:-128}"
    echo "DSv4 AITER MoE trace: topk1=${AITER_DSV4_MOE_DETERMINISTIC_TOPK1}, stage2_set_topk1=${AITER_DSV4_MOE_STAGE2_SET_TOPK1}, trace_set=${AITER_DSV4_MOE_TRACE_SET}, row_check=${AITER_DSV4_MOE_ROW_CHECK}, canary=${AITER_DSV4_MOE_CANARY}, raw_scale=${AITER_DSV4_MOE_TRACE_RAW_SCALE}, abort_after_scale=${AITER_DSV4_MOE_ABORT_AFTER_SCALE_TRACE}, require_prodlike=${AITER_DSV4_MOE_TRACE_REQUIRE_PRODLIKE}, prodlike_min_m=${AITER_DSV4_MOE_PRODLIKE_MIN_M}"
    ...
    if [ "${ATOM_DSV4_PREFILL_DIAG_ENABLE:-1}" = "1" ]; then
        ...
        export ATOM_DSV4_PREFILL_DIAG_PROD_SCAN="${ATOM_DSV4_PREFILL_DIAG_PROD_SCAN:-1}"
        export ATOM_DSV4_PREFILL_DIAG_PROD_LAYERS="${ATOM_DSV4_PREFILL_DIAG_PROD_LAYERS:-all}"
        export ATOM_DSV4_PREFILL_DIAG_TOKENS="${ATOM_DSV4_PREFILL_DIAG_TOKENS:-0,44,-1}"
        export ATOM_DSV4_ROUTER_GATE_TORCH="${ATOM_DSV4_ROUTER_GATE_TORCH:-0}"
        export ATOM_DSV4_ROUTER_GATE_DIAG="${ATOM_DSV4_ROUTER_GATE_DIAG:-1}"
        export ATOM_DSV4_ROUTER_GATE_DIAG_LAYERS="${ATOM_DSV4_ROUTER_GATE_DIAG_LAYERS:-0}"
        export ATOM_DSV4_ROUTER_GATE_DIAG_TOKENS="${ATOM_DSV4_ROUTER_GATE_DIAG_TOKENS:-$ATOM_DSV4_PREFILL_DIAG_TOKENS}"
        export ATOM_DSV4_ROUTER_GATE_FORCE_M="${ATOM_DSV4_ROUTER_GATE_FORCE_M:-8192}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY="${ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY:-0}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY_TOKENS="${ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY_TOKENS:--1}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_REPEAT_MS="${ATOM_DSV4_PREFILL_DIAG_MOE_REPEAT_MS:-}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_FORCE_M="${ATOM_DSV4_PREFILL_DIAG_MOE_FORCE_M:-0}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_ROW_SERIAL="${ATOM_DSV4_PREFILL_DIAG_MOE_ROW_SERIAL:-0}"
        export ATOM_DSV4_PREFILL_DIAG_MOE_ROW_SERIAL_MAX_M="${ATOM_DSV4_PREFILL_DIAG_MOE_ROW_SERIAL_MAX_M:-16}"
        ...
        echo "DSv4 prefill diagnostics enabled: layers=${ATOM_DSV4_PREFILL_DIAG_LAYERS}, deep_layers=${ATOM_DSV4_PREFILL_DIAG_DEEP_LAYERS}, prod_scan=${ATOM_DSV4_PREFILL_DIAG_PROD_SCAN}, prod_layers=${ATOM_DSV4_PREFILL_DIAG_PROD_LAYERS}, prod_tokens=${ATOM_DSV4_PREFILL_DIAG_TOKENS}, sparse_layers=${ATOM_DSV4_PREFILL_DIAG_SPARSE_LAYERS}, moe_replay=${ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY}, moe_replay_tokens=${ATOM_DSV4_PREFILL_DIAG_MOE_REPLAY_TOKENS}, moe_repeat_ms=${ATOM_DSV4_PREFILL_DIAG_MOE_REPEAT_MS}, moe_force_m=${ATOM_DSV4_PREFILL_DIAG_MOE_FORCE_M}, moe_row_serial=${ATOM_DSV4_PREFILL_DIAG_MOE_ROW_SERIAL}, verbose=${ATOM_DSV4_PREFILL_DIAG_VERBOSE}"
        echo "DSv4 router gate diagnostics: torch=${ATOM_DSV4_ROUTER_GATE_TORCH}, diag=${ATOM_DSV4_ROUTER_GATE_DIAG}, layers=${ATOM_DSV4_ROUTER_GATE_DIAG_LAYERS}, tokens=${ATOM_DSV4_ROUTER_GATE_DIAG_TOKENS}, force_m=${ATOM_DSV4_ROUTER_GATE_FORCE_M}"
    fi
fi
