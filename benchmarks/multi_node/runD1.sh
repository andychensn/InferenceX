#!/usr/bin/env bash
# =============================================================================
# runD1.sh — Manual decode node 1 launcher (no Slurm)
#
# Run this on DECODE NODE 1. It starts a decode sglang server, waits for the
# router on the prefill node to come up, then waits for it to shut down
# (signaling end of benchmark) before cleaning up.
#
# Usage:
#   1. Edit the configuration block below (must match runP.sh and runD2.sh)
#   2. Run this script on decode node 1
# =============================================================================
set -euo pipefail
...
DOCKER_CONT_NAME="sglang-disagg-decode1-${RUN_ID}"
...
if [[ -f "$_MORI_PATCH_FILE" ]]; then
    ...
    echo "[runD1] Auto-applied MoRI conn.py overlay"
fi

# NODE_RANK=1: first decode node in a 1P+2D topology
# server.sh assigns rank >= xP*PREFILL_NODES_PER_WORKER to decode role
NODE_RANK=1

echo "============================================="
echo "  Decode Node 1 — run_id=${RUN_ID}"
echo "  This node IP : ${NODE_D1_IP}"
...
docker run --rm \
    ...
    "${IMAGE}" bash -lc "
        ...
        ${SGLANG_WS_PATH}/server.sh 2>&1 | tee /run_logs/slurm_job-${RUN_ID}/server_decode1.log
    "

echo "Decode node 1 finished. Logs in ${LOG_DIR}"
